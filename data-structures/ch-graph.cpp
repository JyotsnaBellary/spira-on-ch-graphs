...
// It is used for CH and CCH for query Processing
    ...
    CH_Graph::CH_Graph(vector<Node> nodes_in_ch, vector<Edge> edges_in_ch, vector<int> rank_order, bool build_adj)
    {
        nodes = nodes_in_ch;
        edges = edges_in_ch;
    ...
    bool CH_Graph::is_up(NodeId u, NodeId v) { return rank[v] > rank[u]; } // your convention

    int CH_Graph::number_of_nodes() const { return static_cast<int>(nodes.size()); }
    const Edge& CH_Graph::get_edge(EdgeId id) const { return edges.at(id); }
    const vector<Edge>& CH_Graph::get_all_edges() const { return edges; }
    const int& CH_Graph::get_number_of_shortcuts() const { return shortcuts; }

    int CH_Graph::get_rank(NodeId node_id) { return rank[node_id];}
    // To query only for upper ranked neighbors
    const unordered_map<NodeId, pair<EdgeId, Weight>>& CH_Graph::up_neighbors(NodeId node) const
    {
    ...
    const vector<Node>& CH_Graph::get_all_nodes() const { return nodes; }
    const Node& CH_Graph::get_node(NodeId id) const { return nodes[id]; }

    int CH_Graph::number_of_edges() const { return static_cast<int>(edges.size()); }

    bool CH_Graph::valid_node(NodeId u) const
    {
        return u >= 0 && u < number_of_nodes();
    }

    // Build adjacency list with only upward edges
    void CH_Graph::build_upward_adj_Lists(vector<int> rank_order)
    {
        for (Edge &edge : edges)
        {
...
void CH_Graph::set_upward_adj_lists(vector<vector<EdgeId>> &out_adj, vector<vector<EdgeId>> &in_adj) {
        out_adjacency_list = out_adj;
        in_adjacency_list = in_adj;
        // upward_edges_adj = out_adjacency_list;
    }

    void CH_Graph::add_to_out_adj(Edge &edge) {
        add_to_out_adj(edge, out_adjacency_list);
    }

    void CH_Graph::add_to_out_adj(Edge &edge, vector<vector<EdgeId>> &adjacency_list)
    {
    ...
    void CH_Graph::add_to_in_adj(Edge &edge) {
        add_to_in_adj(edge, in_adjacency_list);
    }

    void CH_Graph::add_to_in_adj(Edge &edge, vector<vector<EdgeId>> &adjacency_list)
    {
        if (!valid_node(edge.src))
            throw out_of_range("add_edge_adj: invalid node id " + to_string(edge.src));
        if (edge.id < 0 || edge.id >= static_cast<EdgeId>(edges.size()))
    ...
    void CH_Graph::print_upward_adj()
    {
        cout << "upward graph" << endl;
    ...
    void CH_Graph::print_adj_in()
{
    ...
    void CH_Graph::append_unpacked(EdgeId eid,
    ...
    CH_DijkstraResult CH_Graph::unpack_shortcuts(CH_DijkstraResult ch_dijkstraResult)
    {
        vector<EdgeId> edge_out;
    ...
    void CH_Graph::sort_in_neighbors(NodeId nodeId)
    {
        if (nodeId < 0 || nodeId >= number_of_nodes())
            return;
    ...
    void CH_Graph::sort_all_neighbors()
    {
    ...
    EdgeId CH_Graph::get_lowest_ranked_upward_neighbor(NodeId nodeId) const {
        const vector<EdgeId>& neighbors = get_out_neighbors(nodeId);
        EdgeId edge_id = INVALID_EDGE;
        if (neighbors.empty()) 
            return INVALID_EDGE;
        auto lowest_ranked_edge =
        min_element(
            neighbors.begin(),
            neighbors.end(),
            [&](EdgeId a, EdgeId b)
            {
                NodeId trg_a = edges[a].trg;
                NodeId trg_b = edges[b].trg;

                return rank[trg_a] < rank[trg_b];
            });

        return *lowest_ranked_edge;

    }

    vector<NodeId> CH_Graph::find_reachable_nodes(NodeId src, NodeId dst)
    {
    ...
    Query_Graph_Type CH_Graph::get_upward_graph(NodeId src, NodeId dst)
    {
        vector<NodeId> reachable_nodes = find_reachable_nodes(src, dst);
        ...
        query_graph_info.old_node_ids = reachable_nodes;
        ...
        for (NodeId old_node_id: reachable_nodes) {
            ...
            query_graph_info.old_to_new = old_to_new;
            }
        }
