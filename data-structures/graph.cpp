...
Graph::Graph() = default;
    ...
    int  Graph::number_of_nodes() const { return static_cast<int>(nodes.size()); }
    const Node& Graph::get_node(NodeId id) const { return nodes[id]; }
    void  Graph::set_node(NodeId id, const Node &node) { nodes[id] = node; }
    const vector<Node>& Graph::get_all_nodes() const { return nodes; }

    // const vector<NodeId> Graph::get_all_node_ids()
    // {
    ...
    NodeId Graph::get_any_active_node() const
    {
    ...
    uint64_t Graph::key_directed(NodeId u, NodeId v)
    {
        return (uint64_t)u << 32 | (uint32_t)v;
    }

    // --- lookup helper ---
    size_t Graph::edge_index(NodeId src, NodeId dst) const
    {
    ...
    Edge& Graph::get_edge(EdgeId id) { return edges.at(id); }
    ...
    bool Graph::edge_exists(NodeId src, NodeId dst) const
    {
        return edge_pos.find(key_directed(src, dst)) != edge_pos.end();
    }

    Edge& Graph::get_edge(NodeId src, NodeId dst)
    {
        return edges[edge_index(src, dst)];
    }

    // Query edge by src, dst
    const Edge& Graph::get_edge_by_src_dst(NodeId src, NodeId dst) const
    {
    ...
    const vector<Edge>& Graph::get_all_edges() const { return edges; }
    ...
    int Graph::number_of_edges() const { return static_cast<int>(edges.size()); }
    bool Graph::is_active(NodeId v) const { return active[v] != 0; }
    void Graph::deactivate(NodeId v) { active[v] = 0; }

    // Adjacency list helper functions
    int Graph::num_active_neighbors(NodeId u) const
    {
        int count = 0;
    ...
    void Graph::build_adjacency_list(const vector<Edge> *customEdges)
    {
    ...
    EdgeId Graph::add_edge_adj(EdgeId edge_id, NodeId u, NodeId v, Weight w)
    {
        adj[u][v] = {edge_id, w};
        return edge_id;
    }

    const unordered_map<NodeId, pair<EdgeId, Weight>>& Graph::neighbors(NodeId u) const
    {
        return adj[u];
    }

    const void Graph::build_out_in_adjacency_lists()
    {
    ...
    vector<NodeId> Graph::get_sorted_higher_neighbors(NodeId nodeId, vector<int> &node_rank) const
    {
    ...
    int Graph::add_shortcuts(vector<pair<Shortcut, ShortcutOpType>> &shortcuts)
    {
    ...
    void Graph::update_shortcut_info(EdgeId edgeId, EdgeId rev_edgeId, ShortcutInfo shortcutInfo, Weight cost, bool is_shortcut)
    {
