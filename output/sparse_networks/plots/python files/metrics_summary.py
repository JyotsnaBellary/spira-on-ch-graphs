#!/usr/bin/env python3
"""
Metrics summary for sparse graphs.
...
def main() -> None:
    ...
    # CSV outputs
    write_csv(nv_per_m, ROOT / "nv_pops_per_m_sparse.csv")
    write_csv(nv_to_pert, ROOT / "nv_pops_to_pertinent_sparse.csv")
    write_csv(out_in, ROOT / "pertinent_out_in_ratio_sparse.csv")
    write_csv(pert_share, ROOT / "pertinent_share_sparse.csv")
    write_csv(pert_share_n, ROOT / "pertinent_share_n_sparse.csv")
    print(
        f"Written CSVs: {ROOT / 'nv_pops_per_m_sparse.csv'}, "
        f"{ROOT / 'nv_pops_to_pertinent_sparse.csv'}, "
        f"{ROOT / 'pertinent_out_in_ratio_sparse.csv'}, "
        f"{ROOT / 'pertinent_share_sparse.csv'}, "
        f"{ROOT / 'pertinent_share_n_sparse.csv'}"
    )
