#!/usr/bin/env python3
"""
Heatmaps: runtime comparison for sparse graphs across edge-weight distributions.
Reads average.csv from sparse_networks/*/spt_benchmark (and src_dst_benchmark) and
...
def plot_heatmap(matrix: pd.DataFrame, benchmark: str, out_path: Path) -> None:
    ...
    cbar.update_ticks()
    plt.title(f"Sparse runtime (avg) — {benchmark}")
...
def main() -> None:
    ...
    for benchmark in ("spt_benchmark", "src_dst_benchmark"):
        ...
        if matrix.empty:
            ...
            continue
        out = ROOT / f"heatmap_runtime_sparse_{benchmark}.png"
