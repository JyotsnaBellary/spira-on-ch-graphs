#!/usr/bin/env python3
"""Plot edge-relaxation ratios for dense (complete) graphs.
...
random) for both spt_benchmark and src_dst_benchmark and plots:
  - spira_relax/new_variant_relax (ratio)
X-axis uses filenames sorted by n (small→large).
Expects columns: avg_s_pops, avg_nv_pops (rename if different).
"""
...
def load_and_prepare(csv_path: Path) -> pd.DataFrame:
    ...
    # Using PQ pops as the relaxation proxy; adjust here if explicit relax counts exist.
    s_col = "avg_s_pops"
    nv_col = "avg_nv_pops"
...
def plot_for_benchmark(
    ...
    """Plot relaxation ratios for a given benchmark."""
    ...
    ax.set_title(f"Dense graphs ({benchmark}) - pop ratio")
    ax.grid(True, which="both", linestyle=":", alpha=0.4)

    ax.legend(title="Edge weights")

    output_path = root / f"relaxation_ratios_{benchmark}.png"
    fig.tight_layout()
...
def plot_abs_for_benchmark(
    ...
    ax.set_title(f"Dense graphs ({benchmark}) - absolute pops")
    ax.grid(True, which="both", linestyle=":", alpha=0.4)
