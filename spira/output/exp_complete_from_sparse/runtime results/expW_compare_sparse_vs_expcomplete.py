...
#!/usr/bin/env python3
"""
Plot absolute runtimes (not ratios) for exponential-weight graphs:
 - Sparse OSM graphs
...
def plot_exponential_runtime(benchmark: str):
    sparse_root = BASE / "sparse_networks" / "exponential_weights" / benchmark / "average.csv"
    dense_root  = BASE / "exp_complete_from_sparse" / "exponential_weights" / benchmark / "average.csv"
    ...
    plt.title(f"Runtime comparison on exponential-weight graphs ({benchmark})")
    plt.grid(True, which="both", linestyle=":", alpha=0.4)
    plt.legend()

    out = BASE / f"runtime_exponential_{benchmark}.png"
    plt.tight_layout()
