    ...
    const Graph& graph)
{
    ...
    for (NodeId u = 0; u < res.parent.size(); ++u)
    {
        ...
        // if (eid == INVALID_EDGE) continue;  // root or unreachable
        // total_spt_edges++;
...
void run_all_algorithms_on_dense() {
    ...
    for (EdgeId eid = 0; eid < graph.number_of_edges(); ++eid) {
        ...
        if (res2.in_pertinent_edges[eid] && res2.out_pertinent_edges[eid]) {
            cout << "Error: Edge ID " << eid << " is marked both in-pertinent and out-pertinent!" << endl;
        }
        if (res2.in_pertinent_edges_extracted_in_forward_phase[eid] && res2.out_pertinent_edges[eid]) {
            cout << "Err: Edge ID " << eid << " is marked as  out transfered-pertinent and out-pertinent!" << endl;
        }
    }
    ...
    analyze_spt_pertinence(res2, graph);
