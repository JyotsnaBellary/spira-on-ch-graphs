...
DijkstraResult Dijkstra::compute_shortest_path(NodeId src, NodeId dst) {
    ...
    while(pq.size() > 0) {
        ...
        // If we reached the destination, reconstruct the path.
        if (node == dst) {
            return build_path(prev, dist, viaEdge, dst);
        };
...
DijkstraResult Dijkstra::build_path(const vector<int>& prev, const vector<Dist>& dist, const vector<EdgeId>& viaEdge, NodeId dst) 
{
