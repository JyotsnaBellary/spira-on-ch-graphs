...
CH_DijkstraResult CH_Dijkstra::build_ch_path(const vector<int>& prev_forward, const vector<int>& prev_backward, const vector<EdgeId>& viaEdge_forward, const vector<EdgeId>& viaEdge_backward, NodeId best_node, Dist best_dist, int number_of_pops) 
{
    ...
    for (int cur = best_node; cur != -1; cur = prev_forward[cur]) result.ch_path.push_back(cur);
    ...
    for (int cur = best_node; cur != -1; cur = prev_backward[cur]) backward_nodes.push_back(cur);
    ...
    // for (EdgeId edge: result.ch_edge_ids) {
    //     cout << edge << " ";
    // }
    // cout << endl;

    // for (NodeId node: result.ch_path) {
    //     cout << node << " ";
    // }
    // cout << endl;
