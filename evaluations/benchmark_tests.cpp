...
void BenchmarkTests::finalize_aggregate_stats(AggregateStats& agg) {
    ...
    // ------ Pops per m ------
    agg.avg_d_pops_to_m /= R;
    agg.avg_s_pops_to_m /= R;
    agg.avg_nv_pops_to_m /= R;
...
void BenchmarkTests::append_average_summary(const AggregateStats& A, const string& output_csv_path) {
    ...
    if (!exists) {
        out << "filename,n,m,runs,"
            ...
            << "avg_d_pops_to_m,avg_s_pops_to_m,avg_nv_pops_to_m,"
    ...
    out << filename << ","
        ...
        << A.avg_d_pops_to_m << "," << A.avg_s_pops_to_m << "," << A.avg_nv_pops_to_m << ","
...
PertinenceStats BenchmarkTests::analyze_spt_pertinence(
    const SsspResult &res,
    const Query_Graph_Type &reachable_query_graph,
    const CH_Graph &graph)
{
    PertinenceStats stats;
    ...
    // ----- Ratios -----
    if (reachable_query_graph.edge_count > 0)
    {
        // ratio of pertinent edges to total edges in a graph instance
        stats.ratio_pert_m = (double)stats.total_pertinent_edges / (double)reachable_query_graph.edge_count;
    }
    if (reachable_query_graph.reachable_nodes.size() > 0)
    {
        // ratio of pertinent edges to total nodes in a graph instance
        stats.ratio_pert_n = (double)stats.total_pertinent_edges / (double)reachable_query_graph.reachable_nodes.size();
    }
...
PertinenceStats BenchmarkTests::analyze_path_pertinence(const SsspResult &res, const CH_Graph &graph)
{
    PertinenceStats stats;
...
// Analyse and gather pertinence statistics for a path in Query graph
PertinenceStats BenchmarkTests::analyze_path_pertinence(const SsspResult &res, const Query_Graph_Type &reachable_query_graph, const CH_Graph &graph)
{
    PertinenceStats stats;
...
// ---- Main Benchmark ----
void BenchmarkTests::run_src_dst_benchmark_on_CH_graph(CH_Graph &graph, const string &output_csv_path)
{
...
void BenchmarkTests::print_run_stats(NodeId src, const CH_Graph &graph, const Query_Graph_Type &reachable_query_graph,
    ...
    // print results of this run 
        ...
        cout << "Dijkstra: " << dijkstra_time_us << endl;
        cout << "Spira: " << spira_time_us << endl;
        cout << "New Variant: " << new_variant_time_us << endl;

        cout << "================================\n";

        // print number of pops
        cout << "Number of pops: " << endl;
        cout << "Dijkstra node pops: " << result_dijkstra.number_of_pops << endl;
        cout << "Spira: " << result_spira.number_of_pops << endl;
        cout << "New Variant: " << result_new_variant.number_of_pops << endl;
        cout << "New Variant Q pops: " << result_new_variant.number_of_Q_pops << endl;

        cout << "================================\n";

        // print pertinence stats
        cout << "Pertinence Stats: " << endl;
        cout << "In-pertinent edges: " << stats.total_in_pertinent_edges << endl;
        cout << "In-pertinent edges transferred to Q: " << stats.total_in_pertinent_extracted_in_forward << endl;
        cout << "Out-pertinent edges: " << stats.total_out_pertinent_edges << endl;
        cout << "Total pertinent edges: " << stats.total_pertinent_edges << endl;
        cout << "Pertinence conflicts (both in and out): " << stats.conflict_both << endl;
        cout << "SPT edges that are out-pertinent: " << stats.out_spt << endl;
        cout << "SPT edges that are in-pertinent: " << stats.in_spt << endl;
        cout << "SPT edges that are non-pertinent: " << stats.non_pertinent_edge_in_spt << endl;
        cout << "Total edges in SPT: " << stats.total_spt_edges << endl;

        cout << "================================\n";

        cout << "SPT edges incorrectly classified: " << stats.spt_edges_incorrectly_classified << endl;
        cout << "Ratio of pertinent edges to total edges: " << stats.ratio_pert_m << endl;
        cout << "Ratio of pertinent edges to total nodes: " << stats.ratio_pert_n << endl;
        cout << "Ratio of in-pertinent edges transferred to Q: " << stats.ratio_in_transferred << endl;
        cout << "Ratio of out-pertinent edges in SPT: " << stats.ratio_spt_out << endl;
        cout << "Ratio of in-pertinent edges in SPT: " << stats.ratio_spt_in << endl;
        cout << "Ratio of pops that are pertinent: " << stats.ratio_pop_pert << endl;
        cout << "Distance mismatches between Dijkstra and Spira: " << mismatch_count_ds << endl;
        cout << "Distance mismatches between Dijkstra and New Variant: " << mismatch_count_dn << endl;
        cout << "Overall mismatches in SPT results: " << boolalpha << mismatches << endl;
        cout << "Shortcuts in graph: " << graph.shortcuts << endl;
        cout << "---------------------------------------------\n";
}
...
void BenchmarkTests::run_spt_benchmark_on_CH_graph(CH_Graph &graph, const string &output_csv_path)
{
    ...
    out << "src,"
        ...
        << "d_pops,s_pops,nv_pops,nv_q_pops,"
        << "d_avg_pops_per_node,s_avg_pops_per_node,nv_avg_pops_per_node,"
        << "d_avg_pops_to_m,s_avg_pops_to_m,nv_avg_pops_to_m,"
        << "in_pert,in_pert_trans,out_pert,pert,pert_conflicts,"
        << "out_spt,in_spt,non_pert_spt,total_spt,classified_incorrectly,"
        << "ratio_pert_m,ratio_pert_n,ratio_in_transferred,ratio_spt_out,ratio_spt_in,ratio_pop_pert,"
    ...
    for (int src : sources)
    {
        // cout << "Extracting upward graph for source " << src << "..." << endl;
        ...
        PertinenceStats stats = analyze_spt_pertinence(result_new_variant, reachable_query_graph, query_graph);
        ...
        // if (result_new_variant.median != INF_COST) {
        ...
        agg.avg_d_pops_to_m += pops_d_to_m;
        agg.avg_s_pops_to_m += pops_s_to_m;
        agg.avg_nv_pops_to_m += pops_nv_to_m;
        ...
        out << src << ','
            ...
            << result_dijkstra.number_of_pops << ',' << result_spira.number_of_pops << ',' << result_new_variant.number_of_pops << ',' << result_new_variant.number_of_Q_pops << ','
            << result_dijkstra.avg_pops_per_node << ',' << result_spira.avg_pops_per_node << ',' << result_new_variant.avg_pops_per_node << ','
            << pops_d_to_m << ',' << pops_s_to_m << ',' << pops_nv_to_m << ','
            << stats.total_in_pertinent_edges << ',' << stats.total_in_pertinent_extracted_in_forward << ',' << stats.total_out_pertinent_edges << ',' << stats.total_pertinent_edges << ',' << stats.conflict_both << ','
            << stats.out_spt << ',' << stats.in_spt << ',' << stats.non_pertinent_edge_in_spt << ',' << stats.total_spt_edges << ',' << stats.spt_edges_incorrectly_classified << ','
            << stats.ratio_pert_m << ',' << stats.ratio_pert_n << ',' << stats.ratio_in_transferred << ',' << stats.ratio_spt_out << ',' << stats.ratio_spt_in << ',' << stats.ratio_pop_pert << ','
...
void BenchmarkTests::process_sparse_graph_file(const string &filepath, WeightMode weight_mode, string &output_dir)
{
    ...
    string output_csv = output_spt + "/" + filename + ".csv";
