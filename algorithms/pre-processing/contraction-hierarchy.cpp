...
CH::CH(Graph &graph)
    : graph(graph),
      ...
      currentEdgeDiffs(graph.number_of_nodes(), INT_MAX),
      rank_order(graph.number_of_nodes(), INVALID_NODE),
      seen(graph.number_of_nodes(), 0),
      epoch(1) {}
...
int CH::compute_rank_order()
{
    ...
    vector<NodeId> nodesToRecompute(graph.number_of_nodes());
    ...
    {
        int seeded = 0;
        for (NodeId u = 0; u < graph.number_of_nodes() && seeded < limit; ++u)
        {
    ...
    while (contractedNodes < graph.number_of_nodes())
    {
        ...
        if (graph.number_of_nodes() - contractedNodes < 2)
        {
            if (graph.number_of_nodes() - contractedNodes == 0)
                break;
        ...
        size_t remaining = graph.number_of_nodes() - contractedNodes;
    ...
    int errcount = 0;
    for (NodeId nodeId = 0; nodeId < graph.number_of_nodes(); ++nodeId)
    {
