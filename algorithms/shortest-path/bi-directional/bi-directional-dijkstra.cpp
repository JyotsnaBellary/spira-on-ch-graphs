    ...
    bool stop_b)
{
    ...
    return can_f &&
           (!can_b || pq_forward.top().first <= pq_backward.top().first);
}

// turn this into bidirectional djikstra
Bi_Directional_Dijkstra_Result Bi_Directional_Dijkstra::compute_shortest_path(NodeId src, NodeId dst)
{
    ...
    stop_b);
        ...
        if (best_dist < INF &&
            !pq_forward.empty() &&
            !pq_backward.empty() &&
            pq_forward.top().first + pq_backward.top().first >= best_dist)
        {
            break;
        }
    }
